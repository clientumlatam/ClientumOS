      ...
      if (targetEl) {
        ...
        try {
          canvas = await html2canvas(targetEl, {
            ...
            ignoreElements: (element) => {
              if (element === targetEl) return false;
          ...
          console.warn('[PdfExportButton] html2canvas target failed, trying fallback element:', canvasErr);
          const fallbackEl = document.getElementById('brochure-preview-container') || document.querySelector('main') as HTMLElement;
          if (fallbackEl && fallbackEl !== targetEl) {
            canvas = await html2canvas(fallbackEl, {
              scale: 2,
              useCORS: true,
              allowTaint: true,
              backgroundColor: '#ffffff',
        ...
        if (canvas) {
          ...
          for (let i = 1; i <= pageCount; i++) {
            ...
            pdf.setFontSize(7.5);
            pdf.setTextColor(148, 163, 184);
            pdf.text(`${branding.companyName || 'Clientum B2B Intelligence'} · Documento oficial exportado`, margin, pageHeight - 5);
          ...
          // Fallback if canvas capture could not be created
