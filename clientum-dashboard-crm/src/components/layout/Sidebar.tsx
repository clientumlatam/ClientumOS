...
export const Sidebar: React.FC = () => {
  ...
  const commercialNav: Array<{ id: ActiveTab; label: string; icon: React.ElementType; badge?: string | number; badgeColor?: string }> = [
    { id: 'webmail', label: 'Webmail Suite D1', icon: Mail, badge: 'Edge', badgeColor: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' },
  ...
  const operationsNav: Array<{ id: ActiveTab; label: string; icon: React.ElementType }> = [
    { id: 'publicStore', label: 'Tienda Pública & Directorio', icon: Store },
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-5">
        ...
        <div>
          <button
            onClick={() => handleNavClick('gettingStarted')}
            className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold transition-all shadow-sm ${
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <span>Primeros Pasos (Checklist)</span>
            </div>
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-300 font-mono">Onboard</span>
          </button>
        </div>
