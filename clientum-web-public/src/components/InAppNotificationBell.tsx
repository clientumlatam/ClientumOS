...
import { Bell, MessageSquare, UserPlus, CheckCircle2, Trash2, Sparkles, ExternalLink, Radio, ShieldCheck, Zap } from 'lucide-react';
...
export const InAppNotificationBell: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setPushSupported(support.pushManager);
  ...
  const handleTogglePush = async () => {
    ...
    try {
      if (isPushSubscribed) {
        ...
        const res = await subscribeToPushNotifications('Asesor Clientum');
        if (res.success) {
          setIsPushSubscribed(true);
          const newNotif: InAppNotification = {
            id: 'push-sub-' + Date.now(),
            title: '🔔 Notificaciones Push Nativas Activas',
            description: 'Tu dispositivo se ha suscrito con éxito con VAPID keys para recibir alertas de WhatsApp y leads en segundo plano.',
  ...
  const handleTestPush = async () => {
    setTestSent(true);
    await triggerServerTestPush(0, 'Prospecto Test VAPID', '💬 Clientum Push: Alerta en Tiempo Real', 'Probando envío de notificación nativa del sistema vía Service Worker.');
        ...
        {isOpen && (
          <>
            ...
            >
              ...
              {/* Native Web Push Status & Action Banner */}
              <div className="px-4 py-2.5 bg-indigo-50/70 dark:bg-indigo-950/40 border-b border-indigo-100 dark:border-indigo-900/50 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div className={`p-1.5 rounded-lg shrink-0 ${isPushSubscribed ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-400'}`}>
                    <Radio className="w-3.5 h-3.5" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[11px] font-semibold text-slate-800 dark:text-slate-200 truncate">
                      {isPushSubscribed ? 'Push Nativas (VAPID): Activas' : 'Push del Sistema'}
                    </p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">
                      {isPushSubscribed ? 'Service Worker conectado' : 'Recibe leads con la app cerrada'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  {isPushSubscribed ? (
                    <button
                      ...
                      disabled={testSent}
                      className="px-2 py-1 text-[10px] font-semibold bg-emerald-600 hover:bg-emerald-500 text-white rounded-md transition shadow-xs cursor-pointer flex items-center gap-1"
                      title="Probar notificación nativa"
                    >
                      <Zap className="w-3 h-3" />
                      <span>{testSent ? 'Enviando...' : 'Probar'}</span>
                    </button>
                  ) : (
                    <button
                      onClick={handleTogglePush}
                      disabled={isSubscribing || !pushSupported}
                      className="px-2.5 py-1 text-[11px] font-semibold bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-md transition shadow-xs cursor-pointer"
                    >
                      {isSubscribing ? 'Suscribiendo...' : 'Activar Push'}
                    </button>
                  )}
                </div>
              </div>
