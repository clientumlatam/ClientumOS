...
export const CSVStudioView: React.FC = () => {
  ...
  const [pastedData, setPastedData] = useState<string>(
    'Name,Amount,Company,Contact,Stage,Priority\n' +
    'Acme Global Enterprise Cloud,125000,Acme Corp,John Doe,discovery,High\n' +
    'SaaS Analytics License,48000,DataCorp,Sarah Smith,qualified,Medium\n' +
    'ClientumCRM WhatsApp Suite Expansion,35000,OmniConnect,Carlos Mendez,proposal,Critical'
  );
  ...
  const handleParseCSV = () => {
    ...
    if (lines.length < 2) {
      showToast('CSV must include at least 1 header line and 1 data row', 'error');
      return;
    }
    ...
    setParsedRows(rows);
    setIsParsed(true);
    showToast(`Parsed ${rows.length} rows successfully`, 'success');
  };
          ...
          <div className="p-5 rounded-xl border border-[#1e222d] bg-[#0d0f14] space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                2. Paste CSV or Tabular Content
              </label>
              <textarea
                value={pastedData}
                onChange={(e) => {
                ...
                placeholder="Name,Amount,Company,Contact,Stage..."
                className="w-full h-48 px-3.5 py-2.5 rounded-xl bg-[#090b0e] border border-[#222736] text-slate-200 text-xs font-mono leading-relaxed focus:outline-none focus:border-blue-500"
              />
            </div>
          ...
          {isParsed ? (
            ...
            <div className="p-12 rounded-xl border border-dashed border-[#23293a] bg-[#0d0f14]/50 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-xs text-slate-500 max-w-sm mt-1">
                Paste your CSV content on the left panel and click &quot;Parse &amp; Validate Headers&quot; to review the mapped table before importing.
              </p>
            </div>
          )}
        </div>
