...
const searchRegex = /\/\/ 2\. OpenStreetMap Nominatim[\s\S]*?\/\/ 4\. Apify scraper como fallback secundario/g;

const match = searchRegex.exec(code);
if (match) {
  const replacement = `// 2. Gemini Search Grounding en tiempo real (Google Search)
    if (results.length === 0) {
      try {
    ...
    // 3. OpenStreetMap Nominatim (100% Gratuito y sin requerir API Key ni Secretos)
    if (results.length === 0) {
      try {
    ...
    // 4. Apify scraper como fallback secundario`;

  fs.writeFileSync('server.ts', code.replace(match[0], replacement), 'utf8');
  console.log("Successfully swapped OpenStreetMap and Gemini");
} else {
